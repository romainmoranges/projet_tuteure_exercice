...
if (sizeof($_GET)>0){
    ...
    $rVille = implode('',$_GET);
    // echo $_POST;
    $req = $bdd->prepare('SELECT `ville_nom` FROM `villes_france_free` WHERE `ville_nom` LIKE :rVille"%" ORDER BY `ville_population_2012` DESC LIMIT 8');
    ...
    foreach ($villes as $ville) {
        array_push($tabVilles, array("ville" => $ville['ville_nom'] ));
    }
